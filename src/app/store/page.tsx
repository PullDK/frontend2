...
type Cosmetics = {
  ...
  // Novos cosméticos da modal de perfil
...
export default function StorePage() {
  ...
  const ITEMS: Item[] = useMemo(() => [
    ...
    // Novos temas neon/glass/outline/wireframe e padrões
    ...
    // -------- Cosméticos da Modal: Acento (cores do header) --------
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        setCoins(typeof data?.coins === "number" ? data.coins : 500); // saldo inicial se não existir
  ...
  async function equipItem(item: Item) {
    ...
    try {
      ...
      // feedback imediato no chat via localStorage + evento
    ...
    return (
      <div className="rounded border border-border bg-surface p-4 space-y-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {items.map((it) => (
            <div key={it.id} className="rounded border border-border p-3 flex items-center gap-3">
              <div className="min-w-[3rem]">{it.preview}</div>
              ...
              <div className="flex items-center gap-2">
                {!owned(it.id) ? (
                  <button className="px-3 py-1 rounded border border-border hover:bg-white/5" onClick={() => buyItem(it)}>Comprar</button>
                ) : isEquipped(it) ? (
                  <span className="px-3 py-1 rounded bg-primary text-white text-xs">Equipado</span>
                ) : (
                  <button className="px-3 py-1 rounded bg-primary text-white" onClick={() => equipItem(it)}>Equipar</button>
                )}
              </div>
            </div>
          ))}
  ...
  const filteredItems = useMemo(() => ITEMS.filter(i => i.category === activeCategory), [ITEMS, activeCategory]);
  const totalPages = Math.ceil(filteredItems.length / itemsPerPage) || 1;
  const pageItems = useMemo(() => filteredItems.slice((page - 1) * itemsPerPage, (page) * itemsPerPage), [filteredItems, page]);

  useEffect(() => { setPage(1); }, [activeCategory]);

  return (
    <div className="p-6 h-screen overflow-hidden flex flex-col space-y-6">
      <style jsx>{`
        ...
        @keyframes breath { 0%, 100% { transform: scale(1); filter: saturate(1); } 50% { transform: scale(1.02); filter: saturate(1.3); } }
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-semibold">Loja de Cosméticos</h1>
        <div className="flex items-center gap-3">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-500/10 ring-1 ring-amber-500/40 text-amber-300 text-sm" title="Seu saldo">
            <span className="font-semibold">Moedas:</span>
            <span className="font-semibold">{coins}</span>
          </div>
          <button
            className="px-3 py-2 rounded border border-transparent bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-200 ring-1 ring-emerald-500/40"
            onClick={dailyClaim}
            title="Coletar +50 moedas hoje"
          >
            Recompensa diária +50
          </button>
          <Link href="/chat" className="px-3 py-2 rounded border border-border hover:bg-white/5 text-sm" title="Voltar ao chat">
            Voltar ao chat
          </Link>
        </div>
      </div>

      {info && <div className="rounded-md border border-amber-500/30 bg-amber-500/10 p-2 text-xs text-amber-200">{info}</div>}

      {/* Prévia do seu visual atual */}
      <div className="rounded border border-border bg-surface p-4 space-y-3">
        <div className="text-lg font-medium">Sua prévia</div>
      ...
      {/* Conteúdo com navegação por categorias e paginação sem rolagem */}
      <div className="rounded border border-border bg-surface p-4 flex-1 overflow-hidden flex flex-col gap-3">
        <div className="flex items-center gap-2">
          {[
            ...
            { key: "modalBg", label: "Fundo da modal", icon: LayoutGrid },
            { key: "modalFrame", label: "Moldura da modal", icon: SquareDashedMousePointer },
            { key: "modalAccent", label: "Acento da modal", icon: Palette },
          ].map((tab) => (
            <button
              key={tab.key}
              onClick={() => setActiveCategory(tab.key as any)}
              className={
                "px-3 py-2 rounded border text-sm inline-flex items-center gap-2 " +
          ...
          <div className="ml-auto text-xs text-muted">Página {page} de {totalPages}</div>
        </div>
          ...
          {pageItems.map((it) => (
            <div key={it.id} className="rounded-lg border border-border p-3 hover:bg-white/5 transition-colors h-[120px] flex flex-col justify-between" title={it.label}>
              ...
              <div className="flex items-center gap-2 justify-end mt-3">
                {!owned(it.id) ? (
                  <button
                    className="px-3 py-1 rounded border border-border hover:bg-white/5 inline-flex items-center gap-2"
                    onClick={() => buyItem(it)}
                  ...
                  <span className="px-3 py-1 rounded bg-emerald-500/20 text-emerald-200 text-xs ring-1 ring-emerald-500/40 inline-flex items-center gap-2">
                    <Check className="w-4 h-4" />
                    <span>Equipado</span>
                  </span>
                ) : (
                  <button className="px-3 py-1 rounded bg-primary text-white inline-flex items-center gap-2" onClick={() => equipItem(it)} title="Equipar">
                    <Check className="w-4 h-4" />
                    <span>Equipar</span>
                  </button>
                )}
        ...
        <div className="flex items-center justify-end gap-2 pt-2">
          <button
            className="px-3 py-1 rounded border border-border hover:bg-white/5 disabled:opacity-50 disabled:cursor-not-allowed"
      ...
      <div className="text-xs text-muted">Compras e equipamentos são salvos no seu perfil e ficam visíveis para outras pessoas.</div>
    </div>
  );
}